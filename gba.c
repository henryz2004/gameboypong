...
void waitForVBlank(void) {
  ...
  //(1)
  while (160 <= SCANLINECOUNTER && SCANLINECOUNTER <= 227) {
...
void drawFullScreenImageDMA(const u16 *image) {
    ...
    DMA[3].cnt = DMA_ON | RESOLUTION | DMA_SOURCE_INCREMENT | DMA_DESTINATION_INCREMENT;
}
...
void fillScreenDMA(volatile u16 color) {
  DMA[3].src = &color;
  DMA[3].dst = videoBuffer;
  DMA[3].cnt = DMA_ON | RESOLUTION | DMA_DESTINATION_INCREMENT | DMA_SOURCE_FIXED;
}
